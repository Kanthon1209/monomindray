/** 省级行政区（与地图 GeoJSON 命名一致）及地级市，供表单下拉 / 补全 */
...
export function citiesForProvince(province: string): string[] {
  const exact = CITIES_BY_PROVINCE[province];
  if (exact) return exact;
  const hit = Object.keys(CITIES_BY_PROVINCE).find(
    (p) => p.includes(province) || province.includes(p.replace(/(省|市|自治区|特别行政区)$/, "")),
  );
  return hit ? CITIES_BY_PROVINCE[hit] : [];
}
